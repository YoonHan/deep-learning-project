...
class grad_inverter:
    def __init__(self, action_bounds):
        ...
        self.prange = tf.constant(
            [x - y for x, y in zip(action_bounds[0], action_bounds[1])], dtype=tf.float32)
        ...
        self.grad_inverter = tf.where(tf.greater(self.act_grad, self.zeros_act_grad_filter),  # return true when self.act_grad > 0
                                      tf.multiply(self.act_grad, self.pdiff_max), tf.multiply(self.act_grad, self.pdiff_min))  # element size multiply.
        # select(A,B,C) if A  is true then select B , if A is false then select C
