...
class CriticNet:
    """ 
        DDPG algorithm 의 critic Q value 계산 
    """
    ...
    def __init__(self, num_states, num_actions):
        ...
        with self.g.as_default():
            ...
            self.cost = tf.pow(self.critic_q_model -
                               self.q_value_in, 2) / BATCH_SIZE
            ...
            self.act_grad_v = tf.gradients(
                self.critic_q_model, self.critic_action_in)
    ...
    def create_critic_net(self, num_states=4, num_actions=1):
        N_HIDDEN_1 = 192
        N_HIDDEN_2 = 32
        critic_state_in = tf.placeholder(
        ...
        # conv1 (?, 420, 240, 64)
        w1_c = tf.Variable(tf.random_normal(
            [5, 5, 3, 64],  mean=0.01, stddev=0.01))
        l1a = tf.nn.relu(tf.nn.conv2d(critic_state_in, w1_c,
        ...
        # conv2 (?, 210, 120, 32)
        w2_c = tf.Variable(tf.random_normal(
            [5, 5, 64, 32], mean=0.001, stddev=0.003))
        l2a = tf.nn.relu(tf.nn.conv2d(
            l1n, w2_c, strides=[1, 1, 1, 1], padding='SAME'))
        l2p = tf.nn.max_pool(l2a, ksize=[1, 2, 2, 1], strides=[
                             1, 2, 2, 1], padding='SAME')
        l2n = tf.nn.lrn(l2p, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)

        # conv3 (?, 105, 60, 3)
        w3_c = tf.Variable(tf.random_normal(
            [5, 5, 32, 3], mean=0.001, stddev=0.003))
        l3a = tf.nn.relu(tf.nn.conv2d(
            l2n, w3_c, strides=[1, 1, 1, 1], padding='SAME'))
        ...
        w4_c = tf.Variable(tf.random_normal(
            [fc_shape, N_HIDDEN_1], mean=0, stddev=0.001))
        l4 = tf.nn.relu(tf.matmul(l3, w4_c))

        # fc2
        w5_c = tf.Variable(tf.random_normal(
            [N_HIDDEN_1, N_HIDDEN_2], mean=0.1, stddev=0.003))
        w5_action_c = tf.Variable(tf.random_normal(
            [num_actions, N_HIDDEN_2], mean=0, stddev=0.001))
        l5 = tf.nn.relu(tf.matmul(l4, w5_c)) + \
            tf.matmul(critic_action_in, w5_action_c)

        #
        w6_c = tf.Variable(tf.random_normal(
            [N_HIDDEN_2, 1], mean=0, stddev=0.5))
    ...
    def update_target_critic(self):
        self.sess.run([
            ...
            self.t_w5_action_c.assign(
                TAU * self.w5_action_c + (1 - TAU) * self.t_w5_action_c)
        ])
